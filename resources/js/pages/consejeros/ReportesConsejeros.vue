        ...
        <div class="my-6 px-4 py-4">
            <v-form class="col-12" ref="formGraficas">
                <div class="row justify-content-between">
                    ...
                    <div class="col-md-6 col-12">
                        <v-select
                            v-model="data.consejero_id"
                            :items="consejeros"
                            item-title="nombrecompleto"
                            item-value="id"
                            label="Seleccione el consejero"
                            variant="solo"
                ...
                try {
                    ...
                    if (response.status === 200) {
                        if (response.data.status === "ok") {
                            this.consejeros = response.data.consejeros
                            this.consejeros.unshift(this.consejero_default)
                            this.$store.commit('setConsejeros', this.consejeros)
                            // this.mostrar = true
                        } else {
            ...
            async generarGrafica() {
                ...
                // if (valid) {
                    try {
                        ...
                        let response = await axios.post('/api/reporte/grafica-consejero', this.data)
                        if (response.status === 200) {
                            if (response.data.status === "ok") {
                                // 
