    ...
    export default defineComponent({
        ...
        methods: {
            ...
            async getCatalogoCitasDelDia() {
                this.loading = true
                try {
                    let response = await axios.get('/api/catalogos/citas-del-dia-consejero')
            ...
            async guardarCambios()
            {
                ...
                if (valid) {
                    Swal.fire({
                        ...
                        preConfirm: async () => {
                            try {
                                let response = await axios.post('/api/citas/guardar-cambios-cita-consejero', this.cita)
            ...
            async verCitas() {
                ...
                if (valid) {
                    try {
                        this.loading = true
                        let response = await axios.post('/api/citas/citas-del-dia-consejero-buscada', this.ver)
                        if(response.status === 200) {
                            if(response.data.status === "ok") {
                                this.$store.commit('setCatalogoCitasDelDia', response.data.citas)
                                this.mostrar = true
